...
export function drawMesh(
  ...
  if (!landmarks || !landmarks.length) {
    ctx.fillStyle = "rgba(255,200,230,0.1)";
    ctx.font = "11px Inter";
  ...
  // Background grid
  ctx.strokeStyle = "rgba(255,60,180,0.015)";
  ctx.lineWidth = 0.5;
  ...
  for (let i = 0; i < FACE_TRIS.length; i += 3) {
    ...
    const hue = g ? 10 + dp * 30 : 300 + dp * 40;
    ctx.strokeStyle = `hsla(${hue},85%,50%,${g ? 0.4 + dp * 0.4 : 0.12 + dp * 0.2})`;
  ...
  if (g && landmarks[1]) {
    ...
    gr.addColorStop(0, "rgba(255,60,80,0.12)");
